...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = (
    <>
      ...
      <Link href="/drawings" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/drawings") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
        <FileText className="w-4 h-4" /><span>Drawing Library</span>
      </Link>
      ...
      <Link href="/assignments" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/assignments") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
        <Users className="w-4 h-4" /><span>Assignments</span>
      </Link>
      ...
      <Link href="/feed" className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors", location.startsWith("/feed") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
        <FileText className="w-4 h-4" /><span>My Feed</span>
      </Link>
      ...
      <Link href="/checklists" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/checklists") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
        <ListChecks className="w-4 h-4" /><span>Checklists</span>
      </Link>
      <Link href="/chat" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/chat") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
        <MessageSquare className="w-4 h-4" /><span>Team Chat</span>
      </Link>
      ...
      {user?.role === "admin" && (
        <Link href="/users" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/users") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
          <UserRoundCog className="w-4 h-4" /><span>Users</span>
        </Link>
      )}
      {user?.role === "admin" && (
        <Link href="/admin" className={cn("flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors", location.startsWith("/admin") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground")}>
    ...
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-background w-full">
      ...
      <div className="flex items-center justify-between border-b bg-sidebar px-4 py-3 md:hidden">
        <div className="flex min-w-0 items-center gap-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-primary"><Layers className="h-4 w-4 text-primary-foreground" /></div>
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-sidebar-foreground">Design Sense Architects</p>
            <p className="truncate text-[10px] uppercase tracking-wider text-muted-foreground">Drawing Library · {displayName}</p>
          </div>
        </div>
      ...
      <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
        <SheetContent side="left" className="w-[min(86vw,320px)] overflow-y-auto bg-sidebar p-0 text-sidebar-foreground">
          <SheetHeader className="border-b border-sidebar-border/50 p-4 text-left">
            <SheetTitle className="text-left text-sidebar-foreground">Design Sense Architects</SheetTitle>
            <p className="text-xs text-muted-foreground">{displayName}</p>
          </SheetHeader>
          <div className="border-b border-sidebar-border/50 p-3"><UniversalSearch /></div>
      ...
      <aside className="hidden w-64 shrink-0 border-r bg-sidebar md:flex md:flex-col">
        ...
        <nav className="flex-1 overflow-y-auto space-y-1 p-3">{navigation}</nav>
        <div className="border-t border-sidebar-border/50 p-3">
          <button type="button" onClick={() => void logout()} className="flex w-full items-center gap-2 rounded-md border border-sidebar-border/70 px-3 py-2 text-left text-xs font-medium text-sidebar-foreground/80 transition-colors hover:bg-sidebar-accent/50 hover:text-sidebar-foreground">
